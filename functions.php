...
function italiawp2_create_breadcrumbs() {
    ...
    if (!is_home() && !is_front_page()) {
        ...
        if (is_category()) {
            ...
            if (get_post_type() != 'post') {
                ...
                if ($post_type && isset($post_type->labels->singular_name)) {
                    ...
                    if (is_array($slug) && !empty($slug['slug'])) {
            ...
            if ($pt_obj instanceof WP_Post_Type && isset($pt_obj->labels->singular_name)) {
                echo $before . esc_html($pt_obj->labels->singular_name) . $after;
            } elseif ($pt_obj instanceof WP_Post_Type && isset($pt_obj->labels->name)) {
                echo $before . esc_html($pt_obj->labels->name) . $after;
            }
        ...
        if (get_query_var('paged')) {
            if (is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_tax() || is_author())
...
add_filter('upload_mimes', 'italiawp2_allow_data_upload_mimes');
function italiawp2_allow_data_upload_mimes($mimes) {
    $mimes['csv'] = 'text/csv';
...
add_filter('wp_check_filetype_and_ext', 'italiawp2_fix_data_upload_filetypes', 99, 5);
function italiawp2_fix_data_upload_filetypes($data, $file, $filename, $mimes, $real_mime) {
    ...
    if (!isset($map[$ext]) || !is_string($file) || !file_exists($file)) {
        return $data;
    }
    $data['ext'] = $ext;
    $data['type'] = $map[$ext];
...
/**
 * Archive title for post types: show only the label (e.g. "Dataset") without the "Archives:"
 * / "Archivi:" prefix added by WordPress core.
 */
add_filter(
